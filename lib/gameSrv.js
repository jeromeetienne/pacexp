	...
	for(var mazeY = 0; mazeY < mazeH; mazeY++){
		...
		for(var mazeX = 0; mazeX < mazeW; mazeX++){
			...
			this.pills.push(new PillSrv({
				position	: {
					x	: ( mazeX - Math.floor(mazeW/2) ) * bodyW,
					y	: ( mazeY - Math.floor(mazeH/2) ) * bodyW,
	...
	this.players.forEach(function(player, i){
		this.pills.forEach(function(pill){
			...
			if( this.pills.length === 0 ){
				this.trigger("gameover", { reason: "game completed" });	
			}
		}.bind(this));
	}.bind(this))
...
WebyMaze.GameSrv.prototype.addPlayer	= function(gameReq, ioClient){
	...
	player.bind('disconnect', function(){
		...
		if( this.players.length === 0 ){
			this.trigger("gameover", { reason: "no more players" });	
		}
	}.bind(this));
