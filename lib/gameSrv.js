...
this.buildContextPROUT();

	this.shoots.forEach(function(shoot){
		//console.log(shoot.buildContextTick())
...
WebyMaze.GameSrv.prototype.buildContextPROUT	= function(){
	var curRenderInfo	= {
		add	: {},
		upd	: {},
		del	: {}
	};
	if( !this.lastRenderInfoFull ){
	...
	this.shoots.forEach(function(shoot){
		...
		var curFull	= shoot.renderInfoFull();
		// if not yet created, put it lastFull.add + curRenderInfo.add 
		var created	= bodyId in lastFull.add;
//console.log("bodyId", bodyId, "created", created)
//console.log("oldFull", oldFull)
//console.log("curFull", curFull)
		if( !created ){
			curRenderInfo.add[bodyId]	= curFull;
		}else if( ShootSrv.renderInfoEqual(oldFull, curFull) === false ){
