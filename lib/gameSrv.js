	...
	this.players.forEach(function(player){
		this.enemies.forEach(function(enemy){
			...
			if( enemy.stage == 'chase' || enemy.stage == 'scatter' ){
				...
				// - it is 200 by default. but this is the twice last score if you killed in the last 5sec
				var score	= this._enemyLastKilledScore	|| 200;
				if( this._enemyLastKilledDate ){
					if( Date.now() - this._enemyLastKilledDate < 5*1000 ){
						score	*= 2;
					}else{
						score	= 200;						
					}
				}
				this._enemyLastKilledDate	= Date.now();
