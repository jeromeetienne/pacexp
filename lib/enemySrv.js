...
return;
	...
	if( wholeSquare ){
		...
		var cardDir		= require('./tmp/mapUtils.js').distMapMazeSrv2ShortestDir(this.maze, srcX, srcY, 10, 2);
...
//console.assert(false);
		if( cardDir === 'north' )	this.moveRot = cardToAngle['south'];
		else if( cardDir === 'west' )	this.moveRot = cardToAngle['west'];
		else if( cardDir === 'east' )	this.moveRot = cardToAngle['east'];
		else if( cardDir === 'south' )	this.moveRot = cardToAngle['north'];
	}

	if( typeof this.moveRot !== 'undefined' ){
