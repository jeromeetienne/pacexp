 ...
 *   - or simply do it with a tween.js
...
WebyMaze.PlayerSrv.prototype.tickMoveCardinalAbsolute	= function()
{
	...
	return this.tickMoveAbsolute(keys)
}
...
WebyMaze.PlayerSrv.prototype.tickMoveGuidedRelative	= function()
{
	...
	var curMove	= null;
	Object.keys(angle).forEach(function(direction){
		if( this.moveRot == angle[direction] ){
			curMove	= direction;
		}
	}.bind(this))
...
console.log("keys before relative", keys)
	...
	Object.keys(keys).forEach(function(key){
		...
		cardDir[absDir]	= keys[key];
	});
	

	keys		= {};
	keys.right	= cardDir.east;
	keys.up		= cardDir.north;
	keys.left	= cardDir.west;
	keys.down	= cardDir.south;

console.log("cardDir", cardDir)
console.log("keys after relative", keys)
//if( keys.right || keys.up || keys.left || keys.down )	console.assert(false);
 ...
 * - remember until either keyup or can do it
*/
WebyMaze.PlayerSrv.prototype.tickMoveAbsolute	= function(keys)
{
	var wishes	= {
		east	: keys.right,
		north	: keys.up,
	...
	// determine current direction
	var curMove	= null;
	if( this.moveRot === angle.right )	curMove	= 'east';
	if( this.moveRot === angle.up )		curMove	= 'north';
	if( this.moveRot === angle.left )	curMove = 'west';
	if( this.moveRot === angle.down )	curMove = 'south';
	...
	var wallFree	= {}
	wallFree.east	= wallIsFree(angle.right);
	wallFree.north	= wallIsFree(angle.up);
	wallFree.west	= wallIsFree(angle.left)
	wallFree.south	= wallIsFree(angle.down);
